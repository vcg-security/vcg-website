if (
  ...
  !process.env.X_CONTENTFUL_LIVE_TOKEN ||
  !process.env.X_CONTENTFUL_PREVIEW_TOKEN
) {
  throw new Error(
    "Missing required environment variables. Please check: X_CONTENTFUL_SPACE_ID, X_CONTENTFUL_LIVE_TOKEN, X_CONTENTFUL_PREVIEW_TOKEN"
  );
}
